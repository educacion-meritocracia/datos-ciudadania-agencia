...
```{r}
alumnos <- read.csv("input/data/cuestAlumnos.csv", sep = "|")
dim(alumnos)
```
...
```{r}
alumnos_mrun <- read.csv("input/data/alumnosFc_mrun.csv", sep = "|")
dim(alumnos_mrun)
```
...
```{r}
data_mineduc <- read.csv("input/data/Rendimiento_2017_20180131_PUBL.csv", sep = ";")
names(data_mineduc)
```
...
```{r}
simce8b2017 <- read_dta("input/data/simce8b2017_rbd_publica_final.dta")
