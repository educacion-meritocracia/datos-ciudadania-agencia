...
#Gráficos
edad <- na.omit(edad)
ggplot(data=edad, aes(edad)) + geom_histogram(aes(y =..density..),binwidth=1, colour="black") + ggtitle("Histograma edad de la muestra (n=5823)") +
...
#Educación del padre o la madre (si hay información para ambas considerar el promedio)
bbdd$educ_pa_ma <- (bbdd$educmadre + bbdd$educpadre)/2
bbdd$educ_pa_ma <-ifelse(is.na(bbdd$educ_pa_ma), bbdd$educmadre, bbdd$educ_pa_ma)
bbdd$educ_pa_ma <-ifelse(is.na(bbdd$educ_pa_ma), bbdd$educpadre, bbdd$educ_pa_ma)
bbdd$educ_pa_ma <- round(bbdd$educ_pa_ma,0) #redondeamos los valores
...
#Libros en el hogar (padres)
bbdd$librospap <- bbdd$pap11
bbdd$librospap <-factor(bbdd$librospap, levels = c(1,2,3,4,5), labels = c("0 a 10", "11 a 25", "26 a 100", "101 a 200", "200 o más"))
...
Se agrega una transformación al puntaje obtenido en la prueba de conocimiento que considera tres subgrupos: 

+ Estudiantes con puntaje superior: un puntaje en la prueba de conocimiento dentro del 25% superior.
+ Estudiantes con puntaje intermedio: estudiantes dentro del 50% restante en la distribución de puntajes en la prueba de conocimiento. 
+ Estudiantes con puntaje inferior: un puntaje en la prueba de conocimiento dentro del 25% inferior.

```{r puntaje2, echo=TRUE, eval=TRUE}
#Puntaje categórico
bbdd$puntajec[bbdd$puntaje<=408.7] <- 1 #25% inferior de la distribución
bbdd$puntajec[bbdd$puntaje>408.7 & bbdd$puntaje<552.5] <- 2 #50% restante
bbdd$puntajec[bbdd$puntaje>=552.5] <- 3 #25% superior de la distribución

#Transformamos a factor
bbdd$puntajec <-factor(bbdd$puntajec, levels = c(1,2,3), labels = c("25% inferior", "50% restante", "25% superior"))
puntajec <- subset(bbdd, select = c("puntajec"))
puntajec <- na.omit(puntajec)

#Frecuencias
puntajec_freq <- puntajec %>%
  group_by(puntajec)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
puntajec_freq$prop <- round(puntajec_freq$prop, digits = 1)
total <- c("Total", 6342, 100)
puntajec_freq <- rbind(puntajec_freq, total)

#Tabla
colnames(puntajec_freq) <- c("Puntaje Categórico", "Observaciones", "%")
datatable(puntajec_freq, rownames=FALSE, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Removemos  
rm(puntajec_freq, puntajec, total)
...
library(corrplot)
corrplot(cor, type="lower", order="FPC", method="number") # lower x bajo diagonal
...
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=5280)",
  axis.title = c("Puntaje", "Interés Político (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=5244)", axis.title = c("Puntaje", "Interés Político (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción interés en política padres y conocimiento cívico (n=5244)", axis.title = c("Interés Político (padres)", "Interés Político (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción interés en política padres y discusión en la sala de clases (n=5244)", axis.title = c("Interés Político (padres)", "Interés Político (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción interés en política padres y clima escolar (n=5244)", axis.title = c("Interés Político (padres)", "Interés Político (alumnos)"), legend.title = "Clima escolar")

#Removemos 
rm(bbdd1,bbdd2, m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11)
```
...
#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=5208)",
  axis.title = c("Puntaje", "Interés Político (alumnos)"), legend.title = "Libros en el hogar")
...
#Frecuencias Estudiantes
dicta_freqal <- dicta %>%
  group_by(dicta$dicta_al)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqal$prop <- round(dicta_freqal$prop, digits = 1)
total <- c("Total", 5453, 100)
dicta_freqal <- rbind(dicta_freqal, total)
#Tabla
colnames(dicta_freqal) <- c("Se justifican las dictaduras por beneficios económicos (estudiantes)", "Observaciones", "%")
datatable(dicta_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
dicta_freqpap <- dicta %>%
  group_by(dicta$dicta_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqpap$prop <- round(dicta_freqpap$prop, digits = 1)
total <- c("Total", 5453, 100)
dicta_freqpap <- rbind(dicta_freqpap, total)
#Tabla
colnames(dicta_freqpap) <- c("Se justifican las dictaduras por beneficios económicos (padres)", "Observaciones", "%")
datatable(dicta_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")
...
```{r dicta2, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
dicta$dicta_al1 <- as.numeric(dicta$dicta_al)
dicta$dicta_pap1 <- as.numeric(dicta$dicta_pap)
dicta2 <- subset(dicta, select = c("dicta_al1", "dicta_pap1"))
cor <- polychoric(dicta2, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Justificación Alumnos", "Justificación Padres")
rownames(cor) <- c("Justificación Alumnos", "Justificación Padres")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(dicta2, aes(x=dicta_pap1,y=dicta_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Justificación Dictadura Padres') +
ylab('Justificación Dictadura Hijos') +
stat_cor(label.x = 3.49999, label.y = 3.5) 

#Removemos
rm(dicta, dicta2, dicta_freqal, dicta_freqpap, total, alumnos, padres, cor)
```

+ **Regresión Múltiple lineal**: Se presentan un modelo que asume la variable dependiente e independiente del modelo como continuas y otro modelo asumiendo dichas variables como categóricas ordinales. 

```{r dicta3, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
...
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=5280)",
  axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=5244)", axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción justificación dictadura padres y conocimiento cívico (n=5244)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción justificación dictadura padres y discusión en la sala de clases (n=5244)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción justificación dictadura padres y clima escolar (n=5244)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Clima escolar")
...
```{r dicta4, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Base de datos con nuestras variables
...
#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4688)",
  axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Libros en el hogar", value.size=0.3)

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4688)", axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Discusión en sala", value.size=0.3)

plot_model(m9, type = "int",
           title = "Interacción justificación dictadura padres y conocimiento cívico (n=4688)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Conocimiento cívico", value.size=0.3)

plot_model(m10, type = "int",
           title = "Interacción justificación dictadura padres y discusión en la sala de clases (n=4688)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Discusión en sala", value.size=0.3)

plot_model(m11, type = "int",
           title = "Interacción justificación dictadura padres y clima escolar (n=4688)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Clima escolar", value.size=0.3)
...
#Frecuencias Estudiantes
dicta_freqal <- dicta %>%
  group_by(dicta$dicta2_al)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqal$prop <- round(dicta_freqal$prop, digits = 1)
total <- c("Total", 5401, 100)
dicta_freqal <- rbind(dicta_freqal, total)
#Tabla
colnames(dicta_freqal) <- c("La concentración del poder solo en uno garantiza el orden y la seguridad (estudiantes)", "Observaciones", "%")
datatable(dicta_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
dicta_freqpap <- dicta %>%
  group_by(dicta$dicta2_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqpap$prop <- round(dicta_freqpap$prop, digits = 1)
total <- c("Total", 5401, 100)
dicta_freqpap <- rbind(dicta_freqpap, total)
#Tabla
colnames(dicta_freqpap) <- c("La concentración del poder solo en uno garantiza el orden y la seguridad (padres)", "Observaciones", "%")
datatable(dicta_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")
...
```{r dicta22, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
dicta$dicta2_al1 <- as.numeric(dicta$dicta2_al)
dicta$dicta2_pap1 <- as.numeric(dicta$dicta2_pap)
dicta2 <- subset(dicta, select = c("dicta2_al1", "dicta2_pap1"))
cor <- polychoric(dicta2, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Concentración del poder (alumnos)", "Concentración del poder (padres)")
rownames(cor) <- c("Concentración del poder (alumnos)", "Concentración del poder (padres)")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(dicta2, aes(x=dicta2_pap1,y=dicta2_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Justificación Dictadura Padres') +
ylab('Justificación Dictadura Hijos') +
...
```{r dicta23, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
...
#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4666)",
  axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4666)", axis.title = c("Puntaje", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción concentración del poder padres y conocimiento cívico (n=4666)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción concentración del poder padres y discusión en la sala de clases (n=4666)", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción concentración del poder padres y clima escolar (n=4666", axis.title = c("Justificación Dictadura (padres)", "Justificación Dictadura (alumnos)"), legend.title = "Clima escolar")
...
```{r dicta24, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Base de datos con nuestras variables
...
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4666)",
  axis.title = c("Puntaje", "Concentración del poder (alumnos)"), legend.title = "Libros en el hogar", value.size=0.3)

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4666)", axis.title = c("Puntaje", "Concentración del poder (alumnos)"), legend.title = "Discusión en sala", value.size=0.3)

plot_model(m9, type = "int",
           title = "Interacción concentración del poder (padres) y conocimiento cívico (n=4666)", axis.title = c("Concentración del poder (padres)", "Concentración del poder (alumnos)"), legend.title = "Conocimiento cívico", value.size=0.3)

plot_model(m10, type = "int",
           title = "Interacción concentración del poder (padres) y discusión en la sala de clases (n=4666)", axis.title = c("Concentración del poder (padres)", "Concentración del poder (alumnos)"), legend.title = "Discusión en sala", value.size=0.3)

plot_model(m11, type = "int",
           title = "Interacción concentración del poder (padres) y clima escolar (n=4666)", axis.title = c("Concentración del poder (padres)", "Concentración del poder (alumnos)"), legend.title = "Clima escolar", value.size=0.3)
...
```{r dicta31, echo=TRUE, eval=TRUE}
#Generamos variables: El gobierno debería cerrar los medios de comunicación que lo critiquen
bbdd$dicta3_al <- bbdd$alp28_6
bbdd$dicta3_pap <- bbdd$pap19_6
dicta <- subset(bbdd, select = c("dicta3_al", "dicta3_pap"))
skim(dicta) %>% pander()

#Removemos missing
dicta <- na.omit(dicta)
dicta$dicta3_al <-factor(dicta$dicta3_al, levels = c(1,2,3,4), labels = c("Muy desacuerdo", "Desacuerdo", "De acuerdo", "Muy de acuerdo"))
dicta$dicta3_pap <-factor(dicta$dicta3_pap, levels = c(1,2,3,4), labels = c("Muy desacuerdo", "Desacuerdo", "De acuerdo", "Muy de acuerdo"))

#Frecuencias Estudiantes
dicta_freqal <- dicta %>%
  group_by(dicta$dicta3_al)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqal$prop <- round(dicta_freqal$prop, digits = 1)
total <- c("Total", 5435, 100)
dicta_freqal <- rbind(dicta_freqal, total)
#Tabla
colnames(dicta_freqal) <- c("El gobierno debería cerrar los medios de comunicación que lo critiquen (estudiantes)", "Observaciones", "%")
datatable(dicta_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
dicta_freqpap <- dicta %>%
  group_by(dicta$dicta3_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqpap$prop <- round(dicta_freqpap$prop, digits = 1)
total <- c("Total", 5435, 100)
dicta_freqpap <- rbind(dicta_freqpap, total)
#Tabla
colnames(dicta_freqpap) <- c("El gobierno debería cerrar los medios de comunicación que lo critiquen (padres)", "Observaciones", "%")
datatable(dicta_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico 
alumnos <- ggplot(data=dicta, aes(dicta$dicta3_al)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Cerrar medios: alumnos (n=5435)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de acuerdo", y = "%") 

padres <- ggplot(data=dicta, aes(dicta$dicta3_pap)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Cerrar medios: padres (n=5435)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de acuerdo", y = "%") 
#Revisamos los gráficos
ggarrange(alumnos, padres, ncol = 2, nrow = 1)
...
```{r dicta32, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
dicta$dicta3_al1 <- as.numeric(dicta$dicta3_al)
dicta$dicta3_pap1 <- as.numeric(dicta$dicta3_pap)
dicta2 <- subset(dicta, select = c("dicta3_al1", "dicta3_pap1"))
cor <- polychoric(dicta2, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Cerrar medios de comunicación (alumnos)", "Cerrar medios de comunicación (padres)")
rownames(cor) <- c("Cerrar medios de comunicación (alumnos)", "Cerrar medios de comunicación (padres)")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(dicta2, aes(x=dicta3_pap1,y=dicta3_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Cerrar medios de comunicación Padres') +
ylab('Cerrar medios de comunicación Hijos') +
stat_cor(label.x = 3.49999, label.y = 3.5) 
...
```{r dicta33, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
...
#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4693)",
  axis.title = c("Puntaje", "Cierre de medios (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4693)", axis.title = c("Puntaje", "Cierre de medios  (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción cierre de medios (padres) y conocimiento cívico (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción cierre de medios (padres) y discusión en la sala de clases (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción cierre de medios (padres) y clima escolar (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Clima escolar")
...
```{r dicta34, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Base de datos con nuestras variables
...
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4693)",
  axis.title = c("Puntaje", "Cierre de medios (alumnos)"), legend.title = "Libros en el hogar", value.size = 0.3)

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4693)", axis.title = c("Puntaje", "Cierre de medios  (alumnos)"), legend.title = "Nivel de discusión en la sala de clases", value.size = 0.3)

plot_model(m9, type = "int",
           title = "Interacción cierre de medios (padres) y conocimiento cívico (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Nivel de conocimiento cívico", value.size = 0.3)

plot_model(m10, type = "int",
           title = "Interacción cierre de medios (padres) y discusión en la sala de clases (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Nivel de discusión en la sala de clases", value.size = 0.3)

plot_model(m11, type = "int",
           title = "Interacción cierre de medios (padres) y clima escolar (n=4693)", axis.title = c("Cierre de medios (padres)", "Cierre de medios (alumnos)"), legend.title = "Clima escolar", value.size = 0.3)
...
```{r dicta41, echo=TRUE, eval=TRUE}
#Generamos variables: Si el presidente no está de acuerdo con el congreso, debería disolverlo
bbdd$dicta4_al <- bbdd$alp28_7
bbdd$dicta4_pap <- bbdd$pap19_7
dicta <- subset(bbdd, select = c("dicta4_al", "dicta4_pap"))
skim(dicta) %>% pander()

#Removemos missing
dicta <- na.omit(dicta)
dicta$dicta4_al <-factor(dicta$dicta4_al, levels = c(1,2,3,4), labels = c("Muy desacuerdo", "Desacuerdo", "De acuerdo", "Muy de acuerdo"))
dicta$dicta4_pap <-factor(dicta$dicta4_pap, levels = c(1,2,3,4), labels = c("Muy desacuerdo", "Desacuerdo", "De acuerdo", "Muy de acuerdo"))

#Frecuencias Estudiantes
dicta_freqal <- dicta %>%
  group_by(dicta$dicta4_al)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqal$prop <- round(dicta_freqal$prop, digits = 1)
total <- c("Total", 5358, 100)
dicta_freqal <- rbind(dicta_freqal, total)
#Tabla
colnames(dicta_freqal) <- c("Si el presidente no está de acuerdo con el congreso, debería disolverlo (alumnos)", "Observaciones", "%")
datatable(dicta_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
dicta_freqpap <- dicta %>%
  group_by(dicta$dicta4_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
dicta_freqpap$prop <- round(dicta_freqpap$prop, digits = 1)
total <- c("Total", 5358, 100)
dicta_freqpap <- rbind(dicta_freqpap, total)
#Tabla
colnames(dicta_freqpap) <- c("Si el presidente no está de acuerdo con el congreso, debería disolverlo (padres)", "Observaciones", "%")
datatable(dicta_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico 
alumnos <- ggplot(data=dicta, aes(dicta$dicta4_al)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Disolver el congreso: alumnos (n=5358)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de acuerdo", y = "%") 

padres <- ggplot(data=dicta, aes(dicta$dicta4_pap)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Disolver el congreso: padres (n=5358)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de acuerdo", y = "%") 
#Revisamos los gráficos
ggarrange(alumnos, padres, ncol = 2, nrow = 1)
...
```{r dicta42, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
dicta$dicta4_al1 <- as.numeric(dicta$dicta4_al)
dicta$dicta4_pap1 <- as.numeric(dicta$dicta4_pap)
dicta2 <- subset(dicta, select = c("dicta4_al1", "dicta4_pap1"))
cor <- polychoric(dicta2, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Disolver el congreso (alumnos)", "Disolver el congreso (padres)")
rownames(cor) <- c("Disolver el congreso (alumnos)", "Disolver el congreso (padres)")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(dicta2, aes(x=dicta4_pap1,y=dicta4_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Disolver el congreso Padres') +
ylab('Disolver el congreso Hijos') +
stat_cor(label.x = 3.49999, label.y = 3.5) 
...
```{r dicta43, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
...
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4693)",
  axis.title = c("Puntaje", "Disolver el congreso (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4693)", axis.title = c("Puntaje", "Disolver el congreso  (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción disolver el congreso (padres) y conocimiento cívico (n=4693)", axis.title = c("Disolver el congreso (padres)", "Disolver el congreso (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción disolver el congreso (padres) y discusión en la sala de clases (n=4693)", axis.title = c("Disolver el congreso (padres)", "Disolver el congreso (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción disolver el congreso (padres) y clima escolar (n=4693)", axis.title = c("Disolver el congreso (padres)", "Disolver el congreso (alumnos)"), legend.title = "Clima escolar")
...
```{r dicta44, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Base de datos con nuestras variables
...
#Frecuencias Estudiantes
demo_freqal <- demo %>%
  group_by(demo$demo1_al)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
demo_freqal$prop <- round(demo_freqal$prop, digits = 1)
total <- c("Total", 5626, 100)
demo_freqal <- rbind(demo_freqal, total)
#Tabla
colnames(demo_freqal) <- c("Las democracias son la mejor forma porque aseguran la participación (alumnos)", "Observaciones", "%")
datatable(demo_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
demo_freqpap <- demo %>%
  group_by(demo$demo1_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
demo_freqpap$prop <- round(demo_freqpap$prop, digits = 1)
total <- c("Total", 5626, 100)
demo_freqpap <- rbind(demo_freqpap, total)
#Tabla
colnames(demo_freqpap) <- c("Las democracias son la mejor forma porque aseguran la participación (padres)", "Observaciones", "%")
datatable(demo_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")
...
```{r demo2, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
demo$demo1_al1 <- as.numeric(demo$demo1_al)
demo$demo1_pap1 <- as.numeric(demo$demo1_pap)
demo1 <- subset(demo, select = c("demo1_al1", "demo1_pap1"))
cor <- polychoric(demo1, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Democracia - participación (alumnos)", "Democracia - participación (padres)")
rownames(cor) <- c("Democracia - participación (alumnos)", "Democracia - participación  (padres)")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(demo1, aes(x=demo1_pap1,y=demo1_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Democracia - participación: Padres') +
ylab('Democracia - participación: Hijos') +
stat_cor(label.x = 3.49999, label.y = 3.5) 

#Removemos
rm(demo, demo1, demo_freqal, demo_freqpap, total, alumnos, padres, cor)
```

+ **Regresión Múltiple lineal**: Se presentan un modelo que asume la variable dependiente e independiente del modelo como continuas y otro modelo asumiendo dichas variables como categóricas ordinales. 

```{r demo3, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
Hmisc::label(bbdd$demo1_al) <- 'Democracia - participación (Alumnos)'
Hmisc::label(bbdd$demo1_pap) <- 'Democracia - participación (Padres)'
#Variable Categórica
...
m2 <- lm(demo1_al~demo1_pap + puntaje, data=bbdd1)
m3 <- lm(demo1_al~demo1_pap + puntaje + sexo, data=bbdd1)
m4 <- lm(demo1_al~demo1_pap + puntaje + sexo + clima_esc, data=bbdd1)
m5 <- lm(demo1_al~demo1_pap + puntaje + sexo + clima_esc + discusion, data=bbdd1)
m6 <- lm(demo1_al~demo1_pap + puntaje + sexo + clima_esc + discusion + libros, data=bbdd1)
m7 <- lm(demo1_al~demo1_pap + sexo + clima_esc + discusion + puntaje*libros,  data=bbdd1)
m8 <- lm(demo1_al~demo1_pap + sexo + clima_esc + libros + puntaje*discusion,  data=bbdd1)
m9 <- lm(demo1_al~demo1_pap + sexo + clima_esc + libros + discusion + demo1_pap*puntaje,  data=bbdd1)
m10 <- lm(demo1_al~demo1_pap + puntaje + sexo + clima_esc + libros + demo1_pap*discusion,  data=bbdd1)
m11 <- lm(demo1_al~demo1_pap + puntaje + sexo + libros + discusion + demo1_pap*clima_esc,  data=bbdd1)

#Tabla con las regresiones
stargazer(m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11, title = "Modelos de regresión lineal para democracia - participación (estudiantes)",order=c(1,2,4,5,7,10:13,3,6,8,9,14:17),
           covariate.labels=c("Democracia - participación (Padre)", "Conocimiento cívico", "Hombre", "Clima Escolar", "Espacios de discusion", "Libros hogar: 11 a 25", "Libros hogar: 26 a 100", "Libros hogar: 101 a 200", "Libros hogar: 200 o más"),
          column.labels=c("Modelo","Modelo","Modelo","Modelo","Modelo", "Modelo", "Modelo", "Modelo", "Modelo", "Modelo", "Modelo"), 
          dep.var.caption  = "Variable Dependiente",
          dep.var.labels   = "El gobierno debe garantizar la libertad de expresión de las personas",
          type ='html', font.size = "small", align = TRUE, omit.stat=c("f", "ser"), column.sep.width = "-15pt")

#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4811)",
  axis.title = c("Puntaje", "Democracia - participación (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4811)", axis.title = c("Puntaje", "Democracia - participación  (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción Democracia - participación (padres) y conocimiento cívico (n=4811)", axis.title = c("Democracia - participación (padres)", "Democracia - participación (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción democracia - participación(padres) y discusión en la sala de clases (n=4811)", axis.title = c("Democracia - participación (padres)", "Democracia - participación (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción democracia - participación (padres) y clima escolar (n=4811)", axis.title = c("Democracia - participacióno (padres)", "Democracia - participación (alumnos)"), legend.title = "Clima escolar")
...
```{r gob, echo=TRUE, eval=TRUE}
#Generamos variables: Si el presidente no está de acuerdo con el congreso, debería disolverlo
bbdd$gob_al <- bbdd$alp27_1
bbdd$gob_pap <- bbdd$pap21_1
gob <- subset(bbdd, select = c("gob_al", "gob_pap"))
skim(gob) %>% pander()

#Removemos missing
gob <- na.omit(gob)
gob$gob_al <-factor(gob$gob_al, levels = c(1,2,3,4), labels = c("Nada", "Algo", "Bastante", "Mucho"))
gob$gob_pap <-factor(gob$gob_pap, levels = c(1,2,3,4), labels = c("Nada", "Algo", "Bastante", "Mucho"))
...
gob_freqal <- gob %>%
  ...
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
gob_freqal$prop <- round(gob_freqal$prop, digits = 1)
total <- c("Total", 5879, 100)
gob_freqal <- rbind(gob_freqal, total)
#Tabla
colnames(gob_freqal) <- c("Confianza en el gobierno (alumnos)", "Observaciones", "%")
datatable(gob_freqal, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Frecuencias padres
gob_freqpap <- gob %>%
  group_by(gob$gob_pap)%>%
  summarize(n=n())%>%
  mutate(prop=n/sum(n)*100) 
gob_freqpap$prop <- round(gob_freqpap$prop, digits = 1)
total <- c("Total", 5879, 100)
gob_freqpap <- rbind(gob_freqpap, total)
#Tabla
colnames(gob_freqpap) <- c("Confianza en el gobierno (padres)", "Observaciones", "%")
datatable(gob_freqpap, rownames=F, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico 
alumnos <- ggplot(data=gob, aes(gob$gob_al)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Confianza en el gobierno: alumnos (n=5626)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de confianza", y = "%") 

padres <- ggplot(data=gob, aes(gob$gob_pap)) + geom_bar(aes(y = ((..count..)/sum(..count..))), stat="count", colour="black") + 
  scale_y_continuous(labels = scales::percent) +
  geom_text(aes(label = scales::percent(((..count..)/sum(..count..))),
                   y= ((..count..)/sum(..count..))), stat= "count", vjust=-.5, size=3.5) +
  ggtitle("Confianza en el gobierno: padres (n=5626)") +
  theme(plot.title = element_text(size = 9),axis.text.x = element_text(size = 9)) +
  labs(x = "Nivel de confianza", y = "%") 
...
```{r gob2, echo=TRUE, eval=TRUE}
#Asociaciones
#Correlaciones Policóricas
gob$gob_al1 <- as.numeric(gob$gob_al)
gob$gob_pap1 <- as.numeric(gob$gob_pap)
gob1 <- subset(gob, select = c("gob_al1", "gob_pap1"))
cor <- polychoric(gob, na.rm=TRUE)
cor <- cor[["rho"]]
cor <- round(cor,2)
#Tabla
colnames(cor) <- c("Confianza - Gobierno (alumnos)", "Confianza - Gobierno (padres)")
rownames(cor) <- c("Confianza - Gobierno (alumnos)", "Confianza - Gobierno (padres)")
datatable(cor, rownames=T, filter="none", options=list(pageLength=10, scro11X=TRUE), style="default", class="compact")

#Gráfico de asociación 
ggplot(gob1, aes(x=gob_pap1,y=gob_al1)) +
geom_point(position=position_jitter(w=0.1,h=0)) +
geom_smooth(method="lm", se=FALSE) +
xlab('Confianza - Gobierno: Padres') +
ylab('Confianza - Gobierno: Hijos') +
stat_cor(label.x = 3.49999, label.y = 3.5) 

#Removemos
rm(gob, gob1, gob_freqal, gob_freqpap, total, alumnos, padres, cor)
```

+ **Regresión Múltiple lineal**: Se presentan un modelo que asume la variable dependiente e independiente del modelo como continuas y otro modelo asumiendo dichas variables como categóricas ordinales. 

```{r gob3, echo=TRUE, eval=TRUE, warning=FALSE, results = "asis"}
#Ajustamos variable
#Variable continua
Hmisc::label(bbdd$gob_al) <- 'Confianza - Gobierno (Alumnos)'
Hmisc::label(bbdd$gob_pap) <- 'Confianza - Gobierno (Padres)'
#Variable Categórica
bbdd$gob_alc <-factor(bbdd$gob_al, levels = c(1,2,3,4), labels = c("Nada", "Algo", "Bastante", "Mucho"))
bbdd$gob_papc <-factor(bbdd$gob_pap, levels = c(1,2,3,4), labels = c("Nada", "Algo", "Bastante", "Mucho"))

#Modelo de regresión múltiple lineal
#Base de datos para estimación
bbdd1 <- subset(bbdd, select = c("gob_al", "gob_pap", "puntaje", "sexo", "clima_esc", "discusion", "educmadre", "educpadre", "libros"))
...
#1. Modelo original 
m1 <- lm(gob_al~gob_pap, data=bbdd1)
m2 <- lm(gob_al~gob_pap + puntaje, data=bbdd1)
m3 <- lm(gob_al~gob_pap + puntaje + sexo, data=bbdd1)
m4 <- lm(gob_al~gob_pap + puntaje + sexo + clima_esc, data=bbdd1)
m5 <- lm(gob_al~gob_pap + puntaje + sexo + clima_esc + discusion, data=bbdd1)
m6 <- lm(gob_al~gob_pap + puntaje + sexo + clima_esc + discusion + libros, data=bbdd1)
m7 <- lm(gob_al~gob_pap + sexo + clima_esc + discusion + puntaje*libros,  data=bbdd1)
m8 <- lm(gob_al~gob_pap + sexo + clima_esc + libros + puntaje*discusion,  data=bbdd1)
m9 <- lm(gob_al~gob_pap + sexo + clima_esc + libros + discusion + gob_pap*puntaje,  data=bbdd1)
m10 <- lm(gob_al~gob_pap + puntaje + sexo + clima_esc + libros + gob_pap*discusion,  data=bbdd1)
m11 <- lm(gob_al~gob_pap + puntaje + sexo + libros + discusion + gob_pap*clima_esc,  data=bbdd1)

#Tabla con las regresiones
stargazer(m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11, title = "Modelos de regresión lineal para confianza en el gobierno (estudiantes)",order=c(1,2,4,5,7,10:13,3,6,8,9,14:17),
           covariate.labels=c("Confianza - Gobierno (Padre)", "Conocimiento cívico", "Hombre", "Clima Escolar", "Espacios de discusion", "Libros hogar: 11 a 25", "Libros hogar: 26 a 100", "Libros hogar: 101 a 200", "Libros hogar: 200 o más"),
          column.labels=c("Modelo","Modelo","Modelo","Modelo","Modelo", "Modelo", "Modelo", "Modelo", "Modelo", "Modelo", "Modelo"), 
          dep.var.caption  = "Variable Dependiente",
          dep.var.labels   = "Confianza en el gobierno",
          type ='html', font.size = "small", align = TRUE, omit.stat=c("f", "ser"), column.sep.width = "-15pt")

#Gráfico de interacciones 
plot_model(m7, type = "int",
           title = "Interacción conocimiento cívico y libros en el hogar (n=4811)",
  axis.title = c("Puntaje", "Confianza - Gobierno (alumnos)"), legend.title = "Libros en el hogar")

plot_model(m8, type = "int",
           title = "Interacción conocimiento cívico y discusión en la sala de clases (n=4811)", axis.title = c("Puntaje", "Confianza - Gobierno  (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m9, type = "int",
           title = "Interacción Confianza - Gobierno (padres) y conocimiento cívico (n=4811)", axis.title = c("Confianza - Gobierno (padres)", "Democracia - participación (alumnos)"), legend.title = "Nivel de conocimiento cívico")

plot_model(m10, type = "int",
           title = "Interacción Confianza - Gobierno (padres) y discusión en la sala de clases (n=4811)", axis.title = c("Confianza - Gobierno (padres)", "Confianza - Gobierno (alumnos)"), legend.title = "Nivel de discusión en la sala de clases")

plot_model(m11, type = "int",
           title = "Interacción Confianza - Gobierno (padres) y clima escolar (n=4811)", axis.title = c("Confianza - Gobierno (padres)", "Confianza - Gobierno (alumnos)"), legend.title = "Clima escolar")
